module Request exposing
    ( addPItoTravelRequest
    , addTraveltoShellRequest
    , createNewPIRequest
    , createNewTravelRequest
    , getDropboxRequest
    , getSinglePIRequest
    , getSingleShellRequest
...
import Shell exposing (DropboxFacet, Shell, ShellDropbox, ShellFacet)
                                    ...
                                    List.map (getMediafromUrl << .url) medialist.contents
                                        ...
                                        |> Task.andThen
                                            (\mediaFacets ->
                                                let
                                                    pi =
                                                        PI.piFromPIFacet piFacet

                                                    audiotyped =
                                                        List.filter (\mediaFacet -> mediaFacet.mediaType == Media.AudioType) mediaFacets

                                                    notaudiotyped =
                                                        List.filter (\mediaFacet -> mediaFacet.mediaType /= Media.AudioType) mediaFacets

                                                    audios =
                                                        List.map Media.audioFromMediaFacet audiotyped

                                                    medias =
                                                        List.map Media.mediaFromMediaFacet notaudiotyped
                                                in
                                    ...
                                    List.map (getSinglePIRequest << .url) pilist.contents
                                        ...
                                        |> Task.andThen
                                            (\pis ->
                                                let
                                                    travel =
                                                        Travel.travelFromTravelFacet travelFacet
                                                in
                                    ...
                                    List.map (getSingleTravelRequest << .url) travellist.contents
                                        ...
                                        |> Task.andThen
                                            (\travels ->
                                                let
                                                    shell =
                                                        Shell.shellFromShellFacet shellFacet
                                                in
                                                case shellFacet.sender of
                                                    Nothing ->
