...
import Color
import Html exposing (Html, a, div, h1, h2, h3, h4, h5, hr, img, label, span, text)
import Html.Attributes exposing (class, href, id, placeholder, src, style)
import Html.Events exposing (onClick, onMouseOut, onMouseOver, onSubmit)
import Http
import Media exposing (Media)
import Message as Message exposing (Message, userDashboardType, view, viewWarningMessage)
import OverButton as OB exposing (OverButton, carouselNextButton, carouselPrevButton)
...
init flags url key =
    let
        ( state, cmd ) =
            Navbar.initialState UpdateNavbar
...
replaceInList list object =
    List.map
        (\obj ->
            if obj.swissNumber == object.swissNumber then
                object
                ...
                RoutePI data ->
                    case data of
                        ...
                        Just ocapUrl ->
                            let
                                getPIinTravel =
                                    \travel -> List.filter (\pi -> pi.swissNumber == ocapUrl) travel.listPI

                                piList =
                                    List.concatMap getPIinTravel model.shell.travelList
    ...
    case msg of
            ...
            case result of
                Ok pi ->
                    let
                        ...
                        newCurrentTravel =
                            { oldCurrentTravel | listPI = replaceInList oldCurrentTravel.listPI pi }

                        oldShell =
                            model.shell

                        newShell =
                            { oldShell | travelList = replaceInList oldShell.travelList newCurrentTravel }
                    in
                    ( { model
                        | currentPI = pi
                ...
                Ok travel ->
                    if travel.swissNumber /= "" then
                        if travel /= model.currentTravel then
                            let
                                updatedTravel =
                                    { travel | accordionState = model.currentTravel.accordionState }

                                mappedList =
                                    replaceInList model.shell.travelList travel

                                oldShell =
                                    model.shell

                                newShell =
                                    { oldShell | travelList = mappedList }
                            in
                            ( { model
        ...
        AddCheckedToTravel swissNumber ->
            let
                ...
                newShell =
                    { oldShell | travelList = List.map (Travel.updateListPI swissNumber model.checked) oldTravelList }
            in
            if model.currentTravel.swissNumber == swissNumber then
                let
                    ...
                    newCurrentTravelPIs =
                        oldCurrentTravel.listPI ++ model.checked

                    newCurrentTravel =
                        { oldCurrentTravel | listPI = newCurrentTravelPIs }
                in
            ...
            case result of
                Ok travel ->
                    let
                        ...
                        newTravelList =
                            model.shell.travelList ++ [ travel ]

                        newShell =
                            { oldShell | travelList = newTravelList }
                    in
                    case Url.fromString (getRootUrl ++ "/elm/shell#" ++ model.shell.swissNumber) of
                        Just url ->
                            updateFromUrl
        ...
        AddUserContact contactType contact ->
            let
                newContact =
                    User.getContactType contactType contact

                filteredContactList =
                    List.filter (User.filterContactType contactType) model.user.contact
...
viewCompleteFormPassword tmpPassword tmpConfirmPassword =
    let
        ...
        passwordCondition =
            String.length userPassword < 12

        passwordOptions =
            if passwordCondition then
                [ Input.danger ]
        ...
        confirmPasswordOptions =
            if confirmPasswordCondition then
                [ Input.danger ]
...
viewNewAccountForm user tmpPassword tmpConfirmPassword =
    let
        ...
        signupConditions =
            user.name /= "" && userPassword /= "" && userPassword == userConfirmPassword
    in
    Form.form []
...
viewBlockTravel swissNumber travel =
    Block.text
        [ if swissNumber == travel.swissNumber then
            class "lightgrey-background"
...
viewListPIDashboard model travel =
    div []
        ...
        , if List.length travel.listContact > 0 then
            viewContact travel
